...
export default ({ subuser }: Props) => {
    ...
    return (
        <GreyRowBox css={tw`mb-2`}>
            ...
            <div
                css={tw`w-10 h-10 rounded-full bg-white border border-zinc-400 dark:border-zinc-600 overflow-hidden hidden md:block`}
            >
            ...
            <div css={tw`ml-4`}>
                <p css={tw`flex justify-center p-[3px]`}>
                    {subuser.twoFactorEnabled ? (
                        <LockClosedIcon css={tw`w-4 h-4`} />
                    ) : (
                        <LockOpenIcon css={tw`w-4 h-4 text-red-600 dark:text-red-400`} />
                    )}
                </p>
                <p css={tw`text-2xs text-zinc-600 dark:text-zinc-500 uppercase hidden md:block`}>2FA Enabled</p>
            </div>
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center`}>
                    {subuser.permissions.filter((permission) => permission !== 'websocket.connect').length}
                </p>
                <p css={tw`text-2xs text-zinc-600 dark:text-zinc-500 uppercase`}>Permissions</p>
            </div>
            {subuser.uuid !== uuid && (
                <>
