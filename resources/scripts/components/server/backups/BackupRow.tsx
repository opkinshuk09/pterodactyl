...
import { ArchiveIcon, DotsVerticalIcon, LockClosedIcon } from '@heroicons/react/solid';
    ...
    return (
        <GreyRowBox css={tw`flex-wrap md:flex-nowrap items-center`} className={className}>
            <div css={tw`flex items-center truncate w-full md:flex-1`}>
                <div css={tw`mr-4`}>
                    {backup.completedAt !== null ? (
                        backup.isLocked ? (
                            <LockClosedIcon css={tw`w-5 h-5 text-yellow-600 dark:text-yellow-500`} />
                        ) : (
                            <ArchiveIcon css={tw`w-5 h-5 text-yellow-600 dark:text-yellow-500`} />
                        )
                    ) : (
                        <Spinner size={'small'} />
                    )}
            ...
            <div css={tw`flex-1 md:flex-none md:w-48 mt-4 md:mt-0 md:ml-8 md:text-center`}>
                ...
                <p css={tw`text-2xs text-zinc-600 dark:text-zinc-500 uppercase mt-1`}>Created</p>
            </div>
