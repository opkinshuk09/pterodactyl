...
import { ArchiveIcon, DocumentDuplicateIcon, DocumentTextIcon, FolderIcon } from '@heroicons/react/solid';
    ...
    >
        ...
        <Clickable file={file}>
            <div css={tw`flex-none text-neutral-400 mr-4`}>
                {file.isFile ? (
                    file.isSymlink ? (
                        <DocumentDuplicateIcon css={tw`w-5 h-5`} />
                    ) : file.isArchiveType() ? (
                        <ArchiveIcon css={tw`w-5 h-5`} />
                    ) : (
                        <DocumentTextIcon css={tw`w-5 h-5`} />
                    )
                ) : (
                    <FolderIcon css={tw`w-5 h-5`} />
                )}
            </div>
